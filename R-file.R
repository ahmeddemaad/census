...
DS_income_age <- left_join(Sum_income,D_join)
DS_income_age %>% distinct() %>% drop_na() -> DS_income_age
DS_income_age %>% filter(age <25) ->DS_under_25
Avg_under_25 <- mean(DS_under_25$`sum(income_c)`)
DS_income_age %>% filter(age >=25 & age <=50) ->DS_from_25_to_50
Avg_from_25_to_50 <- mean(DS_from_25_to_50$`sum(income_c)`)
DS_income_age %>% filter(age> 50 & age<=75) ->DS_above_50_to_75
Avg_above_50_to_75 <- mean(DS_above_50_to_75$`sum(income_c)`)
DS_income_age %>% filter(age >75) ->DS_above_75
...
avarage_edu_lvl_m <- mean(DS_male_total$education.num)
...
avarage_edu_lvl_f <- mean(DS_female_total$education.num)
cat("Avg.Edu.lvl of   male=",avarage_edu_lvl_m)
cat("Avg.Edu.lvl of female=",avarage_edu_lvl_f)
############################ (in view of the income)
male_income <-full_join(DS_income_age,DS_male_total)
male_income %>% drop_na() %>% select(ID,Gender,`sum(income_c)`) ->male_income
avg_male_income <- mean(male_income$`sum(income_c)`)
female_income <-full_join(DS_income_age,DS_female_total)
...
###(3rd)
###############(income&race)
DS_income_age %>% select(ID,`sum(income_c)`) ->DS_join1
